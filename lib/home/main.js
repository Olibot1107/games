...
try {
    ...
    document.cookie.split(';').forEach(c => {
        ...
        }).then(() => { invalidatePlainJsonCache('/plays.json'); fetchPlayCounts(); }).catch(() => {});
    });
} catch (e) {}
...
function renderGames(){
    ...
    let filtered = allGames.filter(g => {
        ...
        const nameDisp = g.name.replace(/[-_.]+/g, ' ').toLowerCase();
        return (nameRaw.includes(search) || nameDisp.includes(search)) &&
    ...
    filtered.sort((a,b)=>{
        ...
        if (sortMode === 'mytime') return (userPlayTime[b.name] || 0) - (userPlayTime[a.name] || 0);
...
function incrementPlayAndNavigate(game, href){
    ...
    .then(() => {
        invalidatePlainJsonCache('/plays.json');
        fetchPlayCounts();
...
function saveEditComment(commentId, newText, commentElement, originalTextDiv, input, actionsDiv) {
    ...
    .then(result => {
        if (result.success) {
            ...
            invalidatePlainJsonCache('/comments.json');
            fetchCommentCounts();
...
if(searchInput){
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if ((e.key === 't' || e.key === 'T') && tag !== 'INPUT' && tag !== 'TEXTAREA' && !e.ctrlKey && !e.metaKey) {
        ...
        const sortKeys = { '1': 'top', '2': 'plays', '3': 'mytime', '4': 'az' };
        if (sortKeys[e.key] && tag !== 'INPUT' && tag !== 'TEXTAREA' && !e.ctrlKey && !e.metaKey) {
            const mode = sortKeys[e.key];
            sortMode = mode;
        ...
        if ((e.key === 'h' || e.key === 'H') && tag !== 'INPUT' && tag !== 'TEXTAREA' && !e.ctrlKey && !e.metaKey) {
