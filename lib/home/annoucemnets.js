(function () {
  ...
  fetch('/announcements.json')
    ...
    .then(items => {
      ...
      visible.forEach(ann => {
        const s = TYPES[ann.type] || TYPES.info;
        const el = document.createElement('div');
        el.style.cssText = `background:${s.bg};border:1px solid ${s.border};border-radius:14px;padding:13px 16px;display:flex;align-items:flex-start;gap:11px;margin-bottom:8px;`;
        ...
        el.innerHTML = `
          ...
          <div style="flex:1;min-width:0">
            <div style="display:flex;align-items:baseline;gap:8px;margin-bottom:3px">
              <span style="font-size:13px;font-weight:600;color:${s.color}">${ann.title}</span>
