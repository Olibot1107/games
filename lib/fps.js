(function() {
  ...
  function installResourceBridge() {
    ...
    if (!window.__resourceBridgeInstalled) {
      if (!document.getElementById('resource-bridge-loader')) {
        const script = document.createElement('script');
        script.id = 'resource-bridge-loader';
        script.src = '/lib/resource-bridge.js?v=1';
        script.async = false;
    ...
    if ('serviceWorker' in navigator && location.protocol.startsWith('http')) {
      navigator.serviceWorker.register('/resource-sw.js').catch(() => {});
    }
  }
